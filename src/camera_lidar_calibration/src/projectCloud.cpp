...
int main(int argc, char **argv) {
    ...
    // 黑色RGB背景
    cv::Mat zeros = cv::Mat::zeros(src_img.rows, src_img.cols, CV_8UC3);
    ...
    for (unsigned int i = 0; i < lidar_datas.size(); ++i) {
        for (unsigned int j = 0; j < lidar_datas[i].point_num; ++j) {
            ...
            circle(zeros, p, 1, Scalar(b, g, r), -1);
            ++myCount;
    ...
    string output_path_zeros = output_path.substr(0, output_path.length() - 7) + "zeros.png";
    cv::imwrite(output_path_zeros, zeros);
    return 0;
}
